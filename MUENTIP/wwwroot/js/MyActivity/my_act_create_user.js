...
// ฟังก์ชันที่ใช้ในการแสดงกิจกรรม
function renderActivities(container, tagList, maxActivities = Infinity, ownerOnly = false) {
    // ลบกิจกรรมที่แสดงออกทั้งหมดก่อน
    container.innerHTML = "";
    ...
    // กรองกิจกรรมเฉพาะที่เจ้าของเป็น currentUser ถ้าผ่านตัวแปร ownerOnly
    const activitiesToShow = ownerOnly
        ? filteredActivities.filter(activity => activity.owner === currentUser.username)
        : filteredActivities;

    // จำกัดจำนวนกิจกรรมที่จะแสดง
    const activitiesToDisplay = activitiesToShow.slice(0, maxActivities);

    // แสดงกิจกรรมที่ผ่านการกรอง
    activitiesToDisplay.forEach(activity => {
        ...
        let time = date_obj.toTimeString().split(":").slice(0, 2).join(":");
...
document.getElementById("createButton").addEventListener("click", function () {
    renderActivities(document.getElementById("container"), [tag1, tag2, tag3, tag4, tag5], Infinity, true);
});